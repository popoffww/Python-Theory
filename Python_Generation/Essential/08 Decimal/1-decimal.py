...
# Сумма всех чисел и 5 самых больших чисел
...
# Сумма наибольшей и наименьшей цифры Decimal числа,
# с учетом числа до точки
num = Decimal(input())
dig = num.as_tuple().digits

# (abs(num) >= 1) - булево значение:
# True = 1, False = 0
# если num < 1, то (min(dig) * (abs(num) >= 1) == 0
sum = max(dig) + min(dig) * (abs(num) >= 1)
