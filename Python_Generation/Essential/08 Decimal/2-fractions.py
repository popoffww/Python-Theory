from fractions import Fraction
from fractions import Fraction as F
from math import factorial, gcd
# Дроби
numbers = ['6.34', '4.08', '3.04', '7.49', '4.45', '5.39', '7.82', '2.76', '0.71', '1.97', '2.54', '3.67', '0.14', '4.29', '1.84', '4.07', '7.26', '9.37', '8.11', '4.30', '7.16', '2.46', '1.27', '0.29', '5.12', '4.02', '6.95', '1.62', '2.26', '0.45', '6.91', '7.39', '0.52', '1.88', '8.38', '0.75', '0.32', '4.81', '3.31', '4.63', '7.84', '2.25', '1.10', '3.35', '2.05', '7.87', '2.40', '1.20', '2.58', '2.46']

for i in range(len(numbers)):
    print(numbers[i], Fraction(numbers[i]), sep=' = ')
# или
for num in numbers:
    print(f'{num} = {Fraction(num)}')

# Max + min
s = '0.78 4.3 9.6 3.88 7.08 5.88 0.23 4.65 2.79 0.90 4.23 2.15 3.24 8.57 0.10 8.57 1.49 5.64 3.63 8.36 1.56 6.67 1.46 5.26 4.83 7.13 1.22 1.02 7.82 9.97 5.40 9.79 9.82 2.78 2.96 0.07 1.72 7.24 7.84 9.23 1.71 6.24 5.78 5.37 0.03 9.60 8.86 2.73 5.83 6.50 0.123 0.00021'

number = [Fraction(i) for i in s.split()]
print(max(number) + min(number))


# Десятичная дробь
n, m = [F(input()) for _ in range(2)]
print(n / m)
# или
print(F(int(input()), int(input())))


# Математика
n, m = input(), input()

print(f'{n} + {m} =', F(n) + F(m))
print(f'{n} - {m} =', F(n) - F(m))
print(f'{n} * {m} =', F(n) * F(m))
print(f'{n} / {m} =', F(n) / F(m))

# Сумма дробей 1
n = int(input())
s = 0
for i in range(1, n + 1):
    s += F(1, i ** 2)

print(s)

# Сумма дробей 2
n = int(input())
s = 0
for i in range(1, n + 1):
    s += F(1, factorial(i))

print(s)
# или
n = int(input())
a = n // 2
b = n - a
# gcd() позволяет находить наибольший общий делитель(НОД) двух чисел
while gcd(a, b) != 1:
    a -= 1
    b += 1
print(F(a, b))


# Наибольшая правильная несократимая дробь с суммой числителя и знаменателя равной n
n = int(input())

lst = []
for i in range(1, n - 1):
    for j in range(i + 1, n):
        k = F(i, j)
        if k.numerator + k.denominator == n:
            lst.append(k)
print(max(lst))


# Все несократимые дроби, заключённые между 0 и 1, знаменатель которых
# не превосходит n
n = int(input())

set1 = set()
for i in range(1, n):
    for j in range(i + 1, n + 1):
        set1.add(F(i, j))

print(*sorted(set1), sep='\n')
